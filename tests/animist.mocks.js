angular.module('animistMocks', ['pgpKeystoreMocks'])
    ...
    .service('mockPgp', function ($rootScope, keystore, $q) {
      ...
      self.pgpKeyId = '32e6aa474db4f922';
    ...
    // ---------------------------  Mock $cordovaBluetoothBLE ----------------------------------------------
    //
    // Emulates responses from whale-island peripheral. $cordovaBluetoothBLE plugin can only run on the device.
    //
    // Per ngCordovaMocks pattern:
    // setting 'throwsSomething' to true will run the rejection callback for a given test.
    // setting 'emulateGetContract' to true notifies w/ subscription and 5 notify packets ending in 'EOF',
    // setting emulateWriteTx to true notifies w/ subscription and one notice that mocks a txHash msg.
    .service('$cordovaBluetoothLE', function ($q, $rootScope, $timeout) {
      ...
      self.read = function (params) {
        ...
        if (self.throwsRead) { defer.reject(generic); } else if (self.emulateGetBlockNumber) {
    ...
    // ----------------------------------------- Mock $cordovaBeacon --------------------------------------------------
    // $cordovaBeacon plugin can only run on the device.
    //
    // Per ngCordovaMocks pattern:
    // setting 'throwsError' to true will run the rejection callback of any method invoked until the test terminates.
